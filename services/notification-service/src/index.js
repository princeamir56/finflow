...
const unimplemented = (name) =>
  wrap(async function unimplemented(_call, callback) {
    callback({
      ...
      message: `${name} — Not yet implemented; will be done on Day 3`
    });
  });

const implementation = {
  GetUserNotifications: unimplemented('GetUserNotifications'),
  MarkAsRead: unimplemented('MarkAsRead'),
  GetUnreadCount: unimplemented('GetUnreadCount')
};

function main() {
  const server = new grpc.Server();
  server.addService(notifProto.NotificationService.service, implementation);
